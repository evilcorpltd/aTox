...
NDK_COMMON_FILES_MISSING_WHILE_NDK_DIR_INSIDE_HOME := $(shell $(foreach f,$(NDK_COMMON_FILES),test -e "$(ANDROID_NDK_HOME)/$(NDK_DIR)/$f" || echo "$f";))

ifeq ($(NDK_COMMON_FILES_MISSING_WHILE_NDK_DIR_INSIDE_HOME),)
ANDROID_NDK_HOME := $(ANDROID_NDK_HOME)/$(NDK_DIR)
else

NDK_COMMON_FILES_MISSING_WHILE_NDK_DIR_INSIDE_HOME_AS_REVISION := $(shell $(foreach f,$(NDK_COMMON_FILES),test -e "$(ANDROID_NDK_HOME)/$(NDK_REVISION)/$f" || echo "$f";))
ifeq ($(NDK_COMMON_FILES_MISSING_WHILE_NDK_DIR_INSIDE_HOME_AS_REVISION),)
ANDROID_NDK_HOME := $(ANDROID_NDK_HOME)/$(NDK_REVISION)
else
NDK_INVALID := true
ANDROID_NDK_HOME := $(SRCDIR)/$(NDK_DIR)
endif
