...
class ToxThread(
    ...
    companion object {
        ...
        const val msgSetStatusMessage = 4
        const val msgSetStatus = 5
    ...
    val handler: Handler by lazy {
        Handler(looper) {
            when (it.what) {
                ...
                msgSetStatusMessage -> {
                    Log.e("ToxThread", "Setting status message")
                    tox.setStatusMessage(it.obj.toString())
                    handler.sendEmptyMessage(msgSave)
                }
                msgSetStatus -> {
                    Log.e("ToxThread", "Setting status")
                    tox.setStatus(it.arg1)
                    handler.sendEmptyMessage(msgSave)
                }
